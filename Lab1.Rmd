...
*Response: It doesn´t seem like a useful plot as its hard to see the differences in the all the pumpkin categories. There is a trend of one type of pumpkin that is sold all year long (question 1), but most of the other ones are just sold around the ~250 day of the year.*
...
Different models have different pre-processing requirements. For instance, least squares requires encoding categorical variables such as month, variety and city_name. This simply involves translating a column with categorical values into one or more numeric columns that take the place of the original.
...
The processed data baked_pumpkins has all its predictors encoded confirming that indeed the pre-processing steps defined as our recipe will work as expected. This makes it harder for you to read but more intelligible for tidymodels. Take a look at how the observations have been mapped to numbers.

*Question 6:* From looking at the baked_pumpkins tibble, how many total cities are represented in the data set?

*Response: Ten unique cities are within the new table of baked_pumpkins.*
...
*Response: The most correlated variable is the package one, as the other two have a negative correlation. This is predictable because the bigger the packages the bigger the price. The other variables can affect the price, but the main one is the size of the package*.
...
*Response: As we changed the package type to discrete numbers the line doesn't fit very well with this conditions. Additionally there are other variables that are highly correlated to the price like variety, which we are not taking it into consideration within this graph.*
